...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (!text) throw 'Format: *.pinterest Kata Kunci/URL*'
	let res = await pinterest(text)
	// if (!res) throw res
	let mime = await lookup(res)
	text.match(URL_REGEX) ?
		await conn.sendMessage(m.chat, { [mime.split('/')[0]]: { url: res }, caption: `Sukses Mengunduh: *${await shortUrl(res)}*` }, { quoted: m }) :
...
async function pinterest(query) {
	if (query.match(URL_REGEX)) {
		...
		if (!data.length) throw `Kata Kunci *"${query}"* Tidak Ditemukan`
