...
async function handler(m, { conn, args, text, usedPrefix, command }) {
  ...
  try {
    ...
    if (user.health < 0 || user.stamina < 0 || user.mana < 0) {
      return m.reply(`*Minimal Stamina/Mana/Health Kamu 100*`);
    }
    ...
    if(!user.skill) return m.reply("Silahkan Pilih Skill Dulu Di *.selectskill*")
    ...
    if (!(m.sender in conn.mission)) {
      ...
      let caption = {
        ...
        "gems": json.gems,
        "text": "Accept The Mission Or Cancel?"
      };

      return conn.sendMessage(m.chat, JSON.stringify(caption), 'conversation', { quoted: m });
    }
  } catch (e) {
    console.error(e);
...
handler.before = async m => {
  ...
  const cooldown = 30 * (1000 * 60); // cooldown timer in milliseconds
  let user = global.db.data.users[m.sender];

  let txt = (m.msg && m.msg.selectedDisplayText) || m.text || '';
